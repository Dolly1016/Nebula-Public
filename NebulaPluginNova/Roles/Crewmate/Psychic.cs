    ...
    public class Ability : AbstractPlayerUsurpableAbility, IGameOperator, IPlayerAbility
    {
        ...
        void OnReportDeadBody(ReportDeadBodyEvent ev)
        {
            ...
            if (ev.Reporter.AmOwner && ev.Reported != null)
            {
                ...
                {
                    cand.Add(("killersColor", Language.Translate("options.role.psychic.message.killersColor").Replace("%COLOR%", Language.Translate(ModSingleton<BalancedColorManager>.Instance.IsLightColor(DynamicPalette.PlayerColors[ev.Reported.MyKiller.PlayerId]) ? "options.role.psychic.message.inner.lightColor" : "options.role.psychic.message.inner.darkColor"))));
