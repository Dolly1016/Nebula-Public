...
public class GhostAndFlashAbility : IGameOperator
{
    ...
    void OnPlayerMurdered(PlayerMurderedEvent ev)
    {
        ...
        {
            new Ghost(ev.DeadBodyPos ?? ev.Dead.Position, GhostDuration, CommonToken, CanSeeGhostInShadow).RegisterPermanently();
...
public class Seer : DefinedSingleAbilityRoleTemplate<Seer.Ability>, HasCitation, DefinedRole
{
    ...
    public class Ability : AbstractPlayerUsurpableAbility, IPlayerAbility
    {
        ...
        {
            if (MyPlayer.IsAlive && NebulaGameManager.Instance!.AllPlayerInfo.All(p => !p.IsDead || p.AmOwner)) new StaticAchievementToken("seer.another1");
        }
    }
}
