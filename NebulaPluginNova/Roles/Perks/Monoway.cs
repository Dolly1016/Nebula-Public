...
internal class Monoway : PerkFunctionalInstance
{
    ...
    static PerkFunctionalDefinition DefRotate = new("monowayR", PerkFunctionalDefinition.Category.NoncrewmateOnly, new PerkDefinition("monowayRotate", 3, 54, new(72,114,85), Virial.Color.ImpostorColor).CooldownText("%CD%", () => RotateCooldown).DurationText("%D%", () => RotateDuration), (def, instance) => new Monoway(def, instance, () => Mathn.Prob(0.5f) ? new(0f, 1f, -1f, 0f) : new(0f, -1f, 1f, 0f), IconType.Rotate, RotateCooldown, RotateDuration));
    ...
    private static RemoteProcess<(IconType iconType, VVector4 vec, float duration)> RpcEffect = new("Monoway", (message, calledByMe) => {
        ...
        if (!calledByMe || (message.iconType is IconType.Rotate ? GeneralConfigurations.MovementRotationPerksAlsoAffectCastersOption : GeneralConfigurations.MovementRestrictionPerksAlsoAffectCastersOption))
        {
            NebulaManager.Instance.StartDelayAction(0.5f, () =>
            {
                ...
                {
                    PlayerModInfo.RpcAttrModulator.Invoke((AmongUsLLImpl.LocalPlayer.PlayerId, new SpeedModulator(1f, message.vec, true, message.duration, false, 0), true));
                }
            });
        }
    });
