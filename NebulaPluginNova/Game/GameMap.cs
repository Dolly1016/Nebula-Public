...
internal class GameMapImpl : GameMap
{
    ...
    bool ILifespan.IsDeadObject => ModSingleton<GameMap>.Instance != this || !AmongUsLLImpl.ShipStatusInstance.AsBoolFast();
    ...
    float GameMap.ShipMinLightRadius
    {
        get
        {
            shipMinLightRadius ??= AmongUsLLImpl.ShipStatusInstance.MinLightRadius;
            return shipMinLightRadius.Value;
        }
    }

    float? shipMaxLightRadius = null;
    float GameMap.ShipMaxLightRadius
    {
        get
        {
            shipMaxLightRadius ??= AmongUsLLImpl.ShipStatusInstance.MaxLightRadius;
            return shipMaxLightRadius.Value;
        }
    }

    float? crewmateLightMod = null;
    float GameMap.CrewmateLightMod
    {
        get
        {
            crewmateLightMod ??= AmongUsLLImpl.GameOptionManagerInstance.CurrentGameOptions.GetFloat(FloatOptionNames.CrewLightMod);
            return crewmateLightMod.Value;
        }
    }

    float? impostorLightMod = null;
    float GameMap.ImpostorLightMod { get { 
            impostorLightMod ??= AmongUsLLImpl.GameOptionManagerInstance.CurrentGameOptions.GetFloat(FloatOptionNames.ImpostorLightMod);
            return impostorLightMod.Value;
        }
    }
