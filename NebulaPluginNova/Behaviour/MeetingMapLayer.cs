...
/*
internal class MeetingMapLayer : MonoBehaviour
{
}


[HarmonyPatch(typeof(MapBehaviour), nameof(MapBehaviour.Show))]
static public class MeetingMapPatch
{
    static public void Postfix(MapBehaviour __instance)
    {
...
public class DiscussionSupport : AbstractModule<Virial.Game.Game>, IGameOperator
{
    ...
    private record LineInfo(int Id, LineRenderer Line, LineRenderer BackLine1, LineRenderer BackLine2, PassiveButton Button, Collider2D Collider)
    {
        ...
        public void SetPlayer(byte playerId, bool dontSkip = false)
        {
            ...
            PlayerId = playerId;
            SetColor(playerId == byte.MaxValue ? Color.gray : Palette.PlayerColors[playerId]);
        ...
        public void OpenPlayerOverlay()
        {
            ...
            if (player != null)
            {
                NebulaManager.Instance.SetHelpWidget(Button, GUI.API.VerticalHolder(Virial.Media.GUIAlignment.Left,
                    ...
                    NebulaGameManager.Instance!.TryGetTitle(PlayerId, out var title) ? GUI.API.RawText(Virial.Media.GUIAlignment.Left, GUI.API.GetAttribute(Virial.Text.AttributeAsset.OverlayTitle), Language.Translate(title.TranslationKey).Sized(80).Color(Palette.PlayerColors[PlayerId])) : null,
        ...
        if(MeetingHud.Instance && ev is MapOpenNormalEvent && !GeneralConfigurations.ProhibitMeetingTool)
        {
            if (!meetingLayer)
            {
                ...
                Tutorial.ShowTutorial(
                            ...
                            .BindHistory("meetingTool")
                            .ShowWhile(() => MeetingHud.Instance && MapBehaviour.Instance && MapBehaviour.Instance.IsOpen && lines.Count == 0 && icons.Count == 0)
                            .AsSimpleTitledTextWidget(Language.Translate("tutorial.variations.meetingTool.title"), Language.Translate("tutorial.variations.meetingTool.caption")));
            }
        }
        else
        {
    ...
    static private IDividedSpriteLoader MeetingIcons = DividedSpriteLoader.FromResource("Nebula.Resources.MeetingToolIcons.png", 100f, 80, 80, true);
    static private IDividedSpriteLoader MeetingPlayerIcons = DividedSpriteLoader.FromResource("Nebula.Resources.MeetingToolPlayerIcons.png", 100f, 80, 80, true);
