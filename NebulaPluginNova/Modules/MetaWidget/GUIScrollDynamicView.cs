...
public class GUIScrollDynamicInnerContent
{
    ...
    /// GUIウィジット
    /// </summary>
    public GUIWidgetSupplier? Widget { get; private init; }
    ...
    public GUIScrollDynamicInnerContent(GUIAlignment alignment, GUIWidgetSupplier widget, float height)
    {
        this.Widget = widget;
        this.Height = height;
...
public class NoSGUIScrollDynamicViewer : MonoBehaviour
{
    static NoSGUIScrollDynamicViewer() => ClassInjector.RegisterTypeInIl2Cpp<NoSGUIScrollDynamicViewer>();
    ...
    void UpdateContents()
    {
        ...
        for (int i = 0; i < contents.Count; i++)
        {
            ...
            if (objects[i] != show)
            {
                if (show)
                {
                    objects[i] = contents[i].Widget?.Invoke().Instantiate(new Anchor(new(
...
public class GUIScrollDynamicView : AbstractGUIWidget
{

    public string? ScrollerTag { get; init; } = null;
    public UnityEngine.Vector2 Size { get; init; }
    public bool WithMask { get; init; } = true;
    public IReadOnlyList<GUIScrollDynamicInnerContent> widgets { get; init; }
    public GUIScrollDynamicView(Virial.Media.GUIAlignment alignment, UnityEngine.Vector2 size, IReadOnlyList<GUIScrollDynamicInnerContent> widgets) : base(alignment)
    {
        this.Size = size;
    ...
    internal override GameObject? Instantiate(Size size, out Size actualSize)
    {
        ...
        //中身を生成 ここから
        var innerHeightSum = widgets.Sum(w => w.Height);
        scroller.SetBounds(new FloatRange(0, Math.Max(0f, innerHeightSum - Size.y)), null);
        scroller.Inner.gameObject.AddComponent<NoSGUIScrollDynamicViewer>().SetUp(widgets, scroller, hitBox, new(innerSize));
        //中身を生成 ここまで
