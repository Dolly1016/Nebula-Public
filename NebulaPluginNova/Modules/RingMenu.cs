    ...
    public void Show(RingMenuElement[] elements, Func<bool> showWhile)
    {
        ...
        for (int i = 0; i < elements.Length; i++)
        {
            ...
            if (widget != null)
            {
                ...
                widget.transform.localPosition = Vector3.right.RotateZ(-180f + (0.5f + (float)i) * 360f / (float)elements.Length) * 0.68f;
        ...
        if (obj)
        {
            ...
            else
            {
                ...
                if (pos.magnitude > 0.3f && PassiveButtonManager.Instance.currentOver && PassiveButtonManager.Instance.currentOver.GetInstanceID() == button.GetInstanceID())
                {
                    ...
                    {
                        ringRenderer.material.SetFloat("_Guage", 1f / (float)length);
                        var unit = 360f / (float)length;
                        ringRenderer.transform.localEulerAngles = new(0f, 0f, -90f - 180f + unit * (float)(nextSelection + 1));
