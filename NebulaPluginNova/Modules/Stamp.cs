...
public static class StampHelpers
{
    ...
    static public Virial.Media.GUIWidget GetStampLabelWidget(this CosmicStamp? stamp, string? prodId, string? additionalText = null)
    {
        ...
        if (additionalText != null)
        {
            ...
            widgets.Add(GUI.API.RawText(GUIAlignment.Left, GUI.API.GetAttribute(AttributeAsset.OverlayContent), additionalText.Color(new UnityEngine.Color(0.7f,0.7f,0.7f)).Sized(80)));
        }
    ...
    static public Virial.Media.GUIWidget GetStampWidget(this CosmicStamp? stamp, string? prodId, int colorId, Virial.Media.GUIAlignment alignment, bool isMasked, float size = 0.6f, GUIClickableAction? onClick = null, GUIWidgetSupplier? overlay = null, Action? onMouseOver = null, Action? onMouseOut = null)
    {
    ...
    public static void ShowStampRingMenu(Func<bool> showWhile)
    {
    ...
    public static void SetStampShowerToUnderHud(Transform tranform, float z, Func<bool>? aliveWhile = null)
    {
        GamePlayer.AllPlayers.Do(p =>
        {
            p.Unbox().SpecialStampShower = PopupStampShower.GetHudShower(p.PlayerId, tranform, z, aliveWhile);
        });
    }
...
internal static class StampManager
{
    ...
    static private DataSaver MySaver = new("Stamps");
    static private StringArrayDataEntry MyStamps = new("Table", MySaver, [
    ...
    public static void AddToTable(string stamp)
    {
        ...
        MyStamps.Value = table.Append(stamp).ToArray();
    }
...
internal class PopupStampShower : IStampShower
{
    ...
    bool withName = false;
    static internal SpriteLoader MeetingBack = SpriteLoader.FromResource("Nebula.Resources.StampBackground.png", 100f);
    static private SpriteLoader ExileBack = SpriteLoader.FromResource("Nebula.Resources.StampBackground_Exiled.png", 100f);
    ...
    static public PopupStampShower GetHudShower(byte playerId, Transform hud, float z, Func<bool>? aliveWhile) => new(ExileBack, new(-0.25f, -0.55f), hud.transform, new(-4f + 8f * (float)playerId / (float)GamePlayer.AllOrderedPlayers.Count, -3f, z), aliveWhile, true);

    public void Show(CosmicStamp stamp, int colorId)
    {
        if (lastShown) GameObject.Destroy(lastShown);
        ...
        {
            var text = GUI.API.RawText(GUIAlignment.Left, GUI.API.GetAttribute(Virial.Text.AttributeAsset.OverlayTitle), GamePlayer.GetPlayer((byte)colorId)?.Name ?? "");
        ...
        if (lastShown && localAnimatedPos != null)
        {
            IEnumerator CoUpdatePos(GameObject myStamp)
            {
                ...
                if(myStamp) GameObject.Destroy(myStamp);
            }
...
internal class ConditionalStampShower : IStampShower
{
    ...
    void IStampShower.Show(Nebula.Modules.CosmicStamp stamp, int colorId)
    {
        if (predicate.Invoke())
...
internal static class LobbyStampShower
{
    ...
    {
        if (lastShown.TryGetValue(player.PlayerId, out var obj) && obj) GameObject.Destroy(obj);
        ...
        IEnumerator CoUpdate()
        {
            ...
            if (!player && myShown) GameObject.Destroy(myShown);
        }
        NebulaManager.Instance.StartCoroutine(CoUpdate().WrapToIl2Cpp());
    }
}
...
internal class ArrowStampShower : IStampShower
{
    ...
    public void Show(CosmicStamp stamp, int colorId)
    {
        if (lastArrow != null) lastArrow.ReleaseIt();
        if (lastShown != null) GameObject.Destroy(lastShown);
