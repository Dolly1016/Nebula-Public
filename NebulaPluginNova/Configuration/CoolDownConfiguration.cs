...
internal class KillCoolDownConfiguration : IRelativeCoolDownConfiguration
{
    ...
    string? IConfiguration.GetDisplayText() => title.GetString() + ": " + (this as IRelativeCoolDownConfiguration).CoolDown + Language.Translate("options.sec") + " " + CurrentCoolDownStr.Color(Color.gray);

    GUIWidgetSupplier IConfiguration.GetEditor() =>
    new HorizontalWidgetsHolder(GUIAlignment.Center,
        GUI.API.Text(GUIAlignment.Center, GUI.API.GetAttribute(Virial.Text.AttributeAsset.OptionsTitleHalf), title),
        GUI.API.HorizontalMargin(0.04f),
        GUI.API.LocalizedButton(GUIAlignment.Center, GUI.API.GetAttribute(Virial.Text.AttributeAsset.OptionsButtonLonger), typeTranslateKeys[coolDownTypeEntry.Value], _ => coolDownTypeEntry.ChangeValue(true, true)),
        GUI.API.HorizontalMargin(0.2f),
        GUI.API.RawButton(GUIAlignment.Center, GUI.API.GetAttribute(AttributeAsset.OptionsButton), "<<", _ => { CurrentEntry.ChangeValue(false, true); NebulaAPI.Configurations.RequireUpdateSettingScreen(); }),
        ...
        GUI.API.RawButton(GUIAlignment.Center, GUI.API.GetAttribute(AttributeAsset.OptionsButton), ">>", _ => { CurrentEntry.ChangeValue(true, true); NebulaAPI.Configurations.RequireUpdateSettingScreen(); })
    );
